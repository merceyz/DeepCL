    ...
    CLWrapper *gradInput)
{
    while (chosenIndex == -1 && currentIndex < num)
    {
        ...
        try
        {
            ...
            const int time = (int)timer.elapsedMicroseconds();
            if (milliseconds[currentIndex] == -1 || milliseconds[currentIndex] > time)
                milliseconds[currentIndex] = time;
            ...
            if (numTries[currentIndex] >= 3)
            {  // we already tried this kernel 3 times, try next kernel
                currentIndex++;
            }
            return;
        }
    ...
    {
        instances[chosenIndex] = BackpropWeights::instanceSpecific(chosenIndex, cl, dim);
    }
